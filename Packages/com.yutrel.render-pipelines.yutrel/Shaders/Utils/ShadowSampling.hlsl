#ifndef YUTREL_SHADOW_SAMPLING_INCLUDED
#define YUTREL_SHADOW_SAMPLING_INCLUDED
...
// Shared receiver query. The existing helpers return shadow amount, not visibility.
float EvaluateCsmShadowVisibility(DirectionalLightShadowData light_shadow_data,
                                  float3 position_WS, float3 normal_WS)
{
    if (light_shadow_data.index < 0 || _DirectionalShadowCascadeCount <= 0 || light_shadow_data.strength <= 0.0f)
    {
        return 1.0f;
    }
    float depth = LinearEyeDepth(position_WS, UNITY_MATRIX_V);
    return saturate(1.0f - GetDirectionalShadowAttenuation(light_shadow_data,
                                                           GetShadowData(position_WS, depth),
                                                           position_WS,
                                                           normal_WS));
}
