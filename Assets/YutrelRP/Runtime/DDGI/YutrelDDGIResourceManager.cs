...
namespace YutrelRP
{
    public sealed class YutrelDDGIResourceManager : IDisposable
    {
        ...
        private Pcg32 probe_ray_rng = new(5489u, 54u);
        private int probe_ray_rotation_frame = -1;
        private Matrix4x4 probe_ray_rotation = Matrix4x4.identity;
            ...
            ResolvedDDGISettings ddgi_settings)
        {
            ...
            UpdateProbeRayRotation(Time.frameCount);
        ...
        private void UpdateProbeRayRotation(int frame_index)
        {
            if (probe_ray_rotation_frame == frame_index)
            {
                return;
            }

            probe_ray_rotation_frame = frame_index;
            probe_ray_rotation = ComputeProbeRayRotation(NextRandom01(), NextRandom01(), NextRandom01());
        }

        private float NextRandom01()
        {
            return (probe_ray_rng.NextUInt() >> 8) * (1.0f / 16777216.0f);
        }

        private static Matrix4x4 ComputeProbeRayRotation(float random0, float random1, float random2)
        {
            var u1 = 2.0f * Mathf.PI * random0;
            var cos1 = Mathf.Cos(u1);
            var sin1 = Mathf.Sin(u1);

            var u2 = 2.0f * Mathf.PI * random1;
        ...
        private struct Pcg32
        {
            ...
            public Pcg32(uint seed, uint sequence)
            {
                state = 0ul;
                inc = ((ulong)sequence << 1) | 1ul;
                NextUInt();
                state += seed;
                NextUInt();
            }
